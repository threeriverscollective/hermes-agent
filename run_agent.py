...
class AIAgent:
    ...
    def run_conversation(
        ...
        # A dispatcher worker or bounded direct HCP invocation receives its
        # task identity through the inherited environment.  Generating a fresh
        # UUID here would split provider-response attestation (bound to the
        # card) from tool execution (scoped to that UUID), so bind the exact
        # host-selected identity and reject a conflicting caller value.
