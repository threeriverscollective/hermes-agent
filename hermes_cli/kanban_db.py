    ...
    with write_txn(conn):
        ...
        for row in todo_rows:
            ...
            if (
                ...
                # Identity/signature/replay failures require explicit repair.
                # Dependency recomputation must not silently re-admit them.
                continue
...
def record_claimed_provider_guard_hold(
    conn: sqlite3.Connection, task: Task, error_code: str
) -> Optional[dict[str, Any]]:
    if (
...
def _hcp_open_worker_input(root_fd: int, name: str) -> int:
    ...
    try:
        ...
        if (
            ...
            or stat_module.S_IMODE(info.st_mode) != 0o400
...
def _hcp_provider_guard_pass_fds(
    ...
    try:
        ...
        raise ProviderGuardSpawnBlocked("HCP_PERMIT_WORKER_INPUT_INVALID") from exc
    root_mode = stat_module.S_IMODE(root_info.st_mode)
    if (
