...
RESPONSE_BINDING_SCHEMA_VERSION = "hermes.hcp.provider-response-binding.v1"
RESPONSE_BINDING_RESULT_SCHEMA_VERSION = (
    "hermes.hcp.provider-response-binding-result.v1"
)
...
class HCPPermitGuard:
    ...
    def bind_provider_response(
        ...
        **_: object,
        ...
        """Persist one exact normalized provider response in HCP."""

        manifest = self._manifest
        if (
            task_id != manifest["task_id"]
            or self._session_id != session_id
            or type(response_observed_at_unix_ms) is not int
            or response_observed_at_unix_ms <= 0
            or _DIGEST.fullmatch(request_sha256) is None
            or _DIGEST.fullmatch(subject_sha256) is None
            or _DIGEST.fullmatch(response_sha256) is None
        ...
        for value in (
            ...
            _text(value, "PROVIDER_RESPONSE_IDENTITY_INVALID")
        ...
        for item in tool_calls:
            if not isinstance(item, Mapping) or set(item) != {
                ...
                raise ProviderRequestBlocked("PROVIDER_RESPONSE_IDENTITY_INVALID")
            call = dict(item)
            _text(call.get("tool_call_id"), "PROVIDER_RESPONSE_IDENTITY_INVALID")
            _text(call.get("tool_name"), "PROVIDER_RESPONSE_IDENTITY_INVALID")
            if _DIGEST.fullmatch(str(call.get("arguments_sha256"))) is None:
                raise ProviderRequestBlocked("PROVIDER_RESPONSE_IDENTITY_INVALID")
        ...
        with self._lock:
            ...
            if now.tzinfo != timezone.utc:
                raise ProviderRequestBlocked("HCP_PERMIT_CLOCK_INVALID")
            issued_at_unix_ms = int(now.timestamp() * 1000)
            if response_observed_at_unix_ms > issued_at_unix_ms:
                raise ProviderRequestBlocked("PROVIDER_RESPONSE_IDENTITY_INVALID")
            nonce = self._nonce()
            unsigned = {
                "schema_version": RESPONSE_BINDING_SCHEMA_VERSION,
                "operation": "bind_provider_response",
                "task_id": task_id,
                "session_id": session_id,
                "turn_id": turn_id,
                "api_request_id": api_request_id,
                ...
                "signing_identity": manifest["peer_identity"],
                "key_id": manifest["peer_key_id"],
                "issued_at_unix_ms": issued_at_unix_ms,
                "nonce": nonce,
            }
            wire = {**unsigned, "signature": _sign(self._peer_private_key, unsigned)}
            if set(wire) != _RESPONSE_BINDING_KEYS:
                raise ProviderRequestBlocked("PROVIDER_RESPONSE_BINDING_INVALID")
            response = self._transport.exchange(wire)
            return self._validate_response_binding_result(response, unsigned)

    def _validate_response_binding_result(
        self,
        response: Mapping[str, object],
        request: Mapping[str, object],
    ) -> dict[str, object]:
        ...
        if (
            not isinstance(response, Mapping)
            or set(response) != _RESPONSE_BINDING_RESULT_KEYS
            or response.get("schema_version")
            != RESPONSE_BINDING_RESULT_SCHEMA_VERSION
            or response.get("operation") != "bind_provider_response_result"
            or response.get("signing_identity") != manifest["server_identity"]
            or response.get("key_id") != manifest["server_key_id"]
            or _NONCE.fullmatch(str(response.get("response_nonce"))) is None
        ):
            raise ProviderRequestBlocked("PROVIDER_RESPONSE_BINDING_INVALID")
        response_nonce = str(response["response_nonce"])
        if response_nonce in self._used_nonces:
            raise ProviderRequestBlocked("PROVIDER_RESPONSE_BINDING_REPLAY")
        unsigned_response = dict(response)
        signature = unsigned_response.pop("signature")
        _verify(self._server_public_key, unsigned_response, signature)
        self._used_nonces.add(response_nonce)
        for key in (
            ...
            if response.get(key) != request.get(key):
                raise ProviderRequestBlocked("PROVIDER_RESPONSE_BINDING_MISMATCH")
        issued_at_unix_ms = response.get("issued_at_unix_ms")
        now_unix_ms = int(self._clock().timestamp() * 1000)
        if (
            type(issued_at_unix_ms) is not int
            or issued_at_unix_ms > now_unix_ms
            or response.get("status") != "RECORDED"
            or response.get("error_code") is not None
            or _DIGEST.fullmatch(str(response.get("binding_sha256"))) is None
        ):
            error_code = response.get("error_code")
            if type(error_code) is str and _ERROR_CODE.fullmatch(error_code):
                raise ProviderRequestBlocked(error_code)
            raise ProviderRequestBlocked("PROVIDER_RESPONSE_BINDING_INVALID")
        binding_receipt_id = response.get("binding_receipt_id")
        _text(binding_receipt_id, "PROVIDER_RESPONSE_BINDING_INVALID")
        return {
            "binding_sha256": response["binding_sha256"],
